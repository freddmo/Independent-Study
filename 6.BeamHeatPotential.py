# Approximate Dynamic Programming (ADP) for Capacitated Vehicle Routing Problem (CVRP) with Residual Gated GCN, Beam Search + Heat + Potential
...
# -------------------- Beam Search Policy with Potential --------------------
class PartialSolution:
    def __init__(self, route, cost, current, capacity, visited, potential):
        ...
        self.potential = potential  # accumulated heat

    def __lt__(self, other):
        return self.potential > other.potential  # higher potential = better
...
# -------------------- Beam Search Implementation (with heat + potential) --------------------
def beam_search(env, model, beam_width=5):
    ...
    beam = [PartialSolution(route=[0], cost=0, current=0, capacity=env.vehicle_capacity, visited=set(), potential=0)]
        ...
        for sol in beam:
            for j, cust in enumerate(env.customers, 1):
                ...
                if demand <= sol.capacity:
                    ...
                    new_potential = sol.potential + h_ij
                    next_sol = PartialSolution(new_route, new_cost, j, new_capacity, new_visited, new_potential)
                    ...
                    new_visited = sol.visited | {j}
                    new_potential = sol.potential + h_depot
                    next_sol = PartialSolution(new_route, new_cost, j, new_capacity, new_visited, new_potential)
                    next_beam.append(next_sol)
    ...
    best = max(beam, key=lambda s: s.potential)
    return best
...
if __name__ == "__main__":
    ...
    print("Total Cost (neg log heat):", round(solution.cost, 2))
    print("Accumulated Potential (sum of heat):", round(solution.potential, 4))
