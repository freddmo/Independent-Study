# Approximate Dynamic Programming (ADP) for Capacitated Vehicle Routing Problem (CVRP) with Residual Gated GCN and Beam Search + Heat
...
# -------------------- Beam Search Implementation (using heat) --------------------
def beam_search(env, model, beam_width=5):
    ...
    with torch.no_grad():
        ...
        heat = torch.sigmoid(logits).numpy()  # interpret as probability
        ...
        for sol in beam:
            for j, cust in enumerate(env.customers, 1):
                ...
                if demand <= sol.capacity:
                    heat_score = heat[sol.current][j]
                    cost = -np.log(max(heat_score, 1e-5))
                    ...
                    # via depot move
                    heat_score = heat[sol.current][0] * heat[0][j] * 0.1
                    cost = -np.log(max(heat_score, 1e-5))
                    new_cost = sol.cost + cost
...
if __name__ == "__main__":
    ...
    print("Total Cost (negative log heat):", round(solution.cost, 2))
